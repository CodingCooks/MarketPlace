...
public class UserController {
    ...
    public ResponseEntity<Void> changePassword(@RequestBody @Valid PasswordChangingDto passwordChangingDto, Errors errors) {
        if (errors.hasErrors()) {
            errors.getAllErrors().forEach(er -> logger.error(er.getDefaultMessage()));
            throw new EntityValidationException(VALIDATION_EXCEPTION_MESSAGE, errors);
        }
        userService.changePassword(passwordChangingDto);
        return ResponseEntity.ok().build();
    }
}
